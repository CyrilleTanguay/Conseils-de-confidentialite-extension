...
//Fonction pour détecter les options pour les notifications de l'extension sur TikTok
function logTabsSpotify(tabs) {
  for (const tab of tabs) {
    function onGot(item) {
      console.log(item);
      if (Object.values(item) == "true") {
        ...
        browser.pageAction.setIcon({
          tabId: tab.id,
          path: "/icons/icone64Spotify.svg",
        });
      } else {
        browser.pageAction.hide(tab.id);
      }
    }
    browser.storage.sync.get("SpotifyCheck").then(onGot, onError);
  }
}
  ...
  for (const tab of tabs) {
    function onGot(item) {
      ...
      if (Object.values(item) == "true") {
        ...
        browser.pageAction.setIcon({
          tabId: tab.id,
          path: "/icons/icone64TikTok.svg",
        });
      } else {
        browser.pageAction.hide(tab.id);
      }
    }
    browser.storage.sync.get("TikTokCheck").then(onGot, onError);
  }
}
//Les options sont appliquées sur tout onglet Twitter
browser.webNavigation.onCommitted.addListener(() => {
